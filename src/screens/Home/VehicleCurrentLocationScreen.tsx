...
<style>html,body,#map{height:100%;width:100%;margin:0;background:#e2e8f0}.focus{width:34px;height:34px;background:#fff;border:2px solid rgba(0,0,0,.2);border-radius:5px;color:#1976d2;font-size:18px;font-weight:bold}</style>
...
const Focus=L.Control.extend({options:{position:'topleft'},onAdd:function(){const button=L.DomUtil.create('button','focus');button.innerHTML='◎';button.title='Focus vị trí hiện tại';L.DomEvent.disableClickPropagation(button);button.onclick=()=>{if(marker){map.flyTo(marker.getLatLng(),17,{duration:.8});marker.openPopup();}};return button;}});new Focus().addTo(map);
...
const formatLocationTime = (value?: string | null) => {
  ...
  return `${String(date.getHours()).padStart(2, "0")}:${String(
    ...
    date.getDate(),
  ).padStart(2, "0")}/${String(date.getMonth() + 1).padStart(2, "0")}/${date.getFullYear()}`;
};
...
export default function VehicleCurrentLocationScreen() {
  ...
  useEffect(() => {
    getPhuongTien<VehicleListResponse>()
      .then((response) => {
        setVehicles(
  ...
  const loadLocation = useCallback(
    async (showLoading: boolean) => {
      ...
      try {
        const response = await getPhuongTienCurrentLocation<CurrentLocationResponse>(
          trackingId,
        );
      ...
      {!selectedVehicle ? (
        ...
        <View style={styles.flexState}>
          <EmptyState
            ...
            title={loadError ? "Không thể tải vị trí" : "Chưa có dữ liệu vị trí"}
        ...
        <>
          <View style={styles.infoCard}>
            <Text style={styles.vehicleName}>{vehicleLabel(selectedVehicle)}</Text>
...
const styles = StyleSheet.create({
  ...
  infoRow: { flexDirection: "row", alignItems: "flex-start", gap: 7, marginTop: 8 },
  ...
  liveText: { color: "#15803D", fontSize: 12, fontWeight: "600", marginLeft: 7 },
