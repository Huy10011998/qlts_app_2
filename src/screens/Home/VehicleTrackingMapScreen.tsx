...
const buildMapHtml = (points: StopPoint[], selectedId?: number) => `<!doctype html>
<html><head><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=5,user-scalable=yes" />
<link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css" />
<style>html,body,#map{height:100%;width:100%;margin:0;background:#e2e8f0}.fit{width:34px;height:34px;background:#fff;border:2px solid rgba(0,0,0,.2);border-radius:5px;font-size:18px}</style>
</head><body><div id="map"></div><script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script>
<script>
const points=${JSON.stringify(points)};const selectedId=${JSON.stringify(selectedId)};
...
const Fit=L.Control.extend({options:{position:'topleft'},onAdd:function(){const b=L.DomUtil.create('button','fit');b.innerHTML='⌖';b.title='Về tất cả điểm dừng';L.DomEvent.disableClickPropagation(b);b.onclick=()=>bounds&&map.fitBounds(bounds,{padding:[35,35]});return b;}});new Fit().addTo(map);
